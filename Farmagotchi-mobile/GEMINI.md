...
- **2026-02-28:** Refactored all modals (`Tasks`, `InsightsModal`, `OptionModal`, `ScanAnalysisModal`) to use `animationType="fade"` combined with `reanimated`'s `SlideInDown`. This ensures the backdrop fades in smoothly while only the content slides up, fixing the jarring "sliding black overlay" effect.
